...
begin
    ...
    while (!$feof(file_handle)) begin
        ...
        if (char == "0" || char == "1") begin
            current_data = {current_data[2:0], (char == "1") ? 1'b1 : 1'b0};
            bit_count = bit_count + 1;
            
            if (bit_count == 4) begin
                test_data[data_count] = {28'b0, current_data}; // 高28位补0
    ...
    if (bit_count > 0 && bit_count < 4) begin
        ...
        current_data = current_data << (4 - bit_count);
        test_data[data_count] = {28'b0, current_data};
        data_count = data_count + 1;
    end
    
    $fclose(file_handle);
    $display("Read %0d 32-bit data words (4-bit actual) from file %s", data_count, file_path);
...
initial begin
    ...
    axi_slave_write_from_file_int32(
        ...
        2,              // 每次传输后的延迟
        2'b10,          // mem_sel选择C矩阵
        "D:/Desktop/testcase/int4_int32/m8n32k16/c_int32_m8n32k16.txt" //文件路径
    );
    ...
    // 测试2：从文件加载A矩阵数据
    $display("\n=== 测试2：从文件加载A矩阵数据 ===");
    apb_config(2'b00, 2'b10, 1'b1, 3'b000, 1'b1);
    ...
    axi_slave_write_from_file(
        ...
        2,              // 每次传输后的延迟
        2'b00,          // mem_sel选择A矩阵
        "D:/Desktop/testcase/int4_int32/m8n32k16/a_int4_m8n32k16.txt" // 文件路径
    );
    ...
    // 测试3：从文件加载B矩阵数据
    $display("\n=== 测试3：从文件加载B矩阵数据 ===");
    apb_config(2'b01, 2'b10, 1'b1, 3'b000, 1'b1);
    ...
    axi_slave_write_from_file(
        ...
        2,              // 每次传输后的延迟
        2'b01,          // mem_sel选择B矩阵
        "D:/Desktop/testcase/int4_int32/m8n32k16/b_int4_m8n32k16.txt" // 文件路径
    );
