...
begin
    ...
    while (!$feof(file_handle)) begin
        ...
        if (char == "0" || char == "1") begin
            current_data = {current_data[6:0], (char == "1") ? 1'b1 : 1'b0};
            bit_count = bit_count + 1;
            
            if (bit_count == 8) begin
                test_data[data_count] = {24'b0, current_data}; // 高24位补0
                data_count = data_count + 1;
                current_data = 8'b0;
                bit_count = 0;
                
                if (data_count >= num_transfers) break;
            end
        end
    end
    
    if (bit_count > 0 && bit_count < 8) begin
        $display("Warning: Last data has only %0d bits, padding with zeros", bit_count);
        current_data = current_data << (8 - bit_count);
        test_data[data_count] = {24'b0, current_data};
        data_count = data_count + 1;
    end
    
    $fclose(file_handle);
    $display("Read %0d 32-bit data words (8-bit actual) from file %s", data_count, file_path);
...
task axi_slave_write_from_file_int32;
    input [31:0] base_addr;
    input integer num_transfers;
    input integer delay;
    input [1:0] mem_sel;
...
begin
    ...
    while (!$feof(file_handle)) begin
        ...
        if (char == "0" || char == "1") begin
            // 将字符转换为bit并添加到当前数据
            current_data = {current_data[30:0], (char == "1") ? 1'b1 : 1'b0};
            bit_count = bit_count + 1;
            
            // 当收集到32位时，存储数据并重置
            if (bit_count == 32) begin
                test_data[data_count] = current_data;
                data_count = data_count + 1;
                current_data = 32'b0;
                bit_count = 0;
    ...
    // 处理最后不足32位的数据
    if (bit_count > 0 && bit_count < 32) begin
        $display("Warning: Last data has only %0d bits, padding with zeros", bit_count);
        current_data = current_data << (32 - bit_count);
        test_data[data_count] = current_data;
        data_count = data_count + 1;
    end
    
    $fclose(file_handle);
    $display("Read %0d 32-bit data words from file %s", data_count, file_path);
...
initial begin
    ...
    // 测试1：从文件加载C矩阵数据
    $display("\n=== 测试1：从文件加载C矩阵数据 ===");
    apb_config(2'b10, 2'b10, 1'b1, 3'b101, 1'b1); 
    
    // 从文件读取数据并写入AXI总线
    axi_slave_write_from_file_int32(
        32'h0000_0000,  // 基地址
        256,            // 传输次数
        2,              // 每次传输后的延迟
        2'b10,          // mem_sel选择C矩阵
        "D:/Desktop/testcase/int8_int32/m8n32k16/c_int32_m8n32k16.mem.txt" //文件路径
    );
    ...
    // 测试2：从文件加载A矩阵数据
    $display("\n=== 测试2：从文件加载A矩阵数据 ===");
    apb_config(2'b00, 2'b10, 1'b1, 3'b001, 1'b1);
    ...
    axi_slave_write_from_file(
        ...
        2,              // 每次传输后的延迟
        2'b00,          // mem_sel选择A矩阵
        "D:/Desktop/testcase/int8_int32/m8n32k16/a_int8_m8n32k16.mem.txt" // 文件路径
    );
    ...
    // 测试3：从文件加载B矩阵数据
    $display("\n=== 测试3：从文件加载B矩阵数据 ===");
    apb_config(2'b01, 2'b10, 1'b1, 3'b001, 1'b1);
    ...
    axi_slave_write_from_file(
        ...
        2,              // 每次传输后的延迟
        2'b01,          // mem_sel选择B矩阵
        "D:/Desktop/testcase/int8_int32/m8n32k16/b_int8_m8n32k16.mem.txt" // 文件路径
    );
