...
localparam C_S_AXI_DATA_WIDTH = 32;//FP32转BF16
localparam MAX_M = 32;
localparam MAX_N = 32;
localparam MAX_K = 8;//权重剪枝 ＋ FP32转BF16
...
initial begin
    ...
    // 测试1：从文件加载C矩阵数据
    $display("\n=== 测试1：从文件加载C矩阵数据 ===");
    apb_config(2'b10, 2'b10, 1'b0, 3'b011, 1'b1); 
    ...
    axi_slave_write_from_file(
        ...
        2,              // 每次传输后的延迟
        2'b10,          // mem_sel选择C矩阵
        "D:/Desktop/testcase/fp32/m8n32k16/structured_pruning/c_fp32_structured_pruning_m8n32k16.mem.txt" //文件路径
    );
    ...
    // 测试2：从文件加载A矩阵数据
    $display("\n=== 测试2：从文件加载A矩阵数据 ===");
    apb_config(2'b00, 2'b10, 1'b0, 3'b011, 1'b1);
    ...
    axi_slave_write_from_file(
        ...
        2,              // 每次传输后的延迟
        2'b00,          // mem_sel选择A矩阵
        "D:/Desktop/testcase/fp32/m8n32k16/structured_pruning/a_fp32_structured_pruning_m8n32k16.mem.txt" // 文件路径
    );
    ...
    // 测试3：从文件加载B矩阵数据
    $display("\n=== 测试3：从文件加载B矩阵数据 ===");
    apb_config(2'b01, 2'b10, 1'b0, 3'b011, 1'b1);
    ...
    axi_slave_write_from_file(
        ...
        2,              // 每次传输后的延迟
        2'b01,          // mem_sel选择B矩阵
        "D:/Desktop/testcase/fp32/m8n32k16/structured_pruning/b_fp32_structured_pruning_m8n32k16.mem.txt" // 文件路径
    );
